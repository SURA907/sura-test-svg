...
public class ECDSASigner implements ECConstants, DSA {
     ...
     * @param message the message that will be verified later.
     */
    public byte[] generateSignatureVRS(byte[] message) {
        ECDomainParameters ec = key.getParameters();
        BigInteger n = ec.getN();
        BigInteger e = calculateE(n, message);
        BigInteger d = ((ECPrivateKeyParameters) key).getD();
        ...
        BigInteger affineXCoordValue = p.normalize().getAffineXCoord().toBigInteger();
        BigInteger affineYCoordValue = p.normalize().getAffineYCoord().toBigInteger();

        int recId = affineYCoordValue.and(BigInteger.ONE).intValue();
        recId |= (affineXCoordValue.compareTo(r) != 0 ? 2 : 0);
        if (s.compareTo(n.shiftRight(1)) > 0) {
            s = n.subtract(s);
            recId = recId ^ 1;
        }

        return new SignatureData(recId + 27, r, s).toBytes();
    }
